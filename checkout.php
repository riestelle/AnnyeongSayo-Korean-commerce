...
// CONSTANTS
// IMG_PATH — adjust the base path to match your project structure.
if (!defined('IMG_PATH')) {
    define('IMG_PATH', 'assets/images/products/');
}

// PRODUCT LOOKUP  (via GET ?id=)
$product_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Simulated product catalogue.
// Replace with a real DB query, e.g.:
//   $stmt = $pdo->prepare("SELECT * FROM products WHERE id = ?");
//   $stmt->execute([$product_id]);
//   $product = $stmt->fetch(PDO::FETCH_ASSOC);
$catalogue = [
    1 => [
        'name'  => 'Cyber-Pop Sticker Pack',
        'price' => 18.00,
        'qty'   => 1,
        'image' => 'cyber-pop-sticker.jpg',
    ],
    2 => [
        'name'  => 'Melon Blast Juice',
        'price' => 12.50,
        'qty'   => 1,
        'image' => 'melon-blast-juice.jpg',
    ],
];
...
$product_price = number_format((float)$product['price'], 2);  // e.g. "18.00"
$product_qty   = (int)$product['qty'];
$product_image = IMG_PATH . htmlspecialchars($product['image']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    if (empty($payment_method)) {
        ...
        // Generate a simple order ID (replace with real DB insert + auto-increment ID)
        $order_id = strtoupper(
            '#' . substr(md5(uniqid((string)$user_id, true)), 0, 2)
                . '-' . substr(md5(uniqid((string)$product_id, true)), 0, 4)
                . '-' . substr(md5(uniqid('', true)), 0, 3)
                . '-' . chr(rand(65, 90))
        );
    }
}
      ...
      <section class="section-bg-surface comic-panel-shadow">
        ...
        <div class="loot-list">
          <div class="loot-item">
            ...
            <div class="loot-info">
              <div class="loot-name"><?= $product_name ?></div>
              <div class="loot-qty">Qty: <?= $product_qty ?></div>
            </div>
            <div class="loot-price">$<?= $product_price ?></div>
          </div>
        </div>
      </section>
      ...
      <section class="final-damage">
        ...
        <div class="final-damage-inner">
          ...
          <div class="final-total-row">
            ...
            <div class="final-total-amount">$<?= $product_price ?></div>
          </div>
        </div>
      </section>
