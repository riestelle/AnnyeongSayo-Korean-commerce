...
body {
  ...
  background-image: radial-gradient(rgba(0,0,0,0.12) 1px, transparent 0); background-size: 14px 14px; 
}
...
<main>
  ...
  <div class="search-filter">
    <div class="search-wrap">
      ...
      <input id="searchInput" placeholder="SEARCH LIMITED EDITIONS..." type="text" oninput="filterProducts()"/>
    </div>
    <div class="select-wrap">
      <select id="categorySelect" onchange="filterProducts()">
        <option value="">CATEGORY: ALL</option>
        <option value="Noodles">NOODLES</option>
        <option value="Chips &amp; Savory Snacks">CHIPS &amp; SAVORY SNACKS</option>
        <option value="Sweets &amp; Desserts">SWEETS &amp; DESSERTS</option>
        <option value="Drinks &amp; Beverages">DRINKS &amp; BEVERAGES</option>
        <option value="Merch &amp; Merchandise">MERCH &amp; MERCHANDISE</option>
      </select>
  ...
  <section class="product-grid">
    ...
    if (!empty($products)) :
      foreach ($products as $product) :
        ...
        $card_index++;
        $img_url    = htmlspecialchars($product['image_url']);
    ...
    <div class="modal-product-grid">
      ...
      <div class="modal-content-panel">
        ...
        <div style="display:flex; gap:0.75rem;">
          <button class="modal-add-btn" id="modal-checkout-btn" onclick="handleCheckout()">
            <span class="material-symbols-outlined">payment</span> CHECKOUT
          </button>
          <button class="modal-cart-btn" id="modal-cart-btn" onclick="handleAddToCart()">
            <span class="material-symbols-outlined">shopping_cart</span>
          </button>
        </div>
        <div id="modal-cart-msg" style="font-size:0.75rem; font-weight:700; text-align:center; margin-top:0.5rem; display:none;"></div>
      </div>
    </div>
    <div class="modal-crosssell">
      <h2 class="modal-crosssell-title">YOU MIGHT ALSO LIKE</h2>
      <div class="modal-crosssell-underline"></div>
      <div class="modal-crosssell-grid" id="crosssell-grid">
        <!-- dynamically populated by loadCrossSell() -->
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  function filterProducts() {
    ...
    let empty = document.getElementById('no-products-msg');
    if (!empty) {
      empty = document.createElement('p');
      empty.id = 'no-products-msg';
      empty.style.cssText = 'grid-column:1/-1;text-align:center;font-weight:700;padding:3rem;';
  ...
  function openModal(productId, title, price, img, desc, badge, sticker, cat, stock, rating, sku) {
    ...
    // reset cart message
    var msg = document.getElementById('modal-cart-msg');
    if (msg) { msg.style.display = 'none'; msg.textContent = ''; }
    document.getElementById('productModal').classList.add('open');
    document.body.style.overflow = 'hidden';
    // load category-based recommendations
    loadCrossSell(productId, cat);
  }

  function loadCrossSell(currentProductId, category) {
    var grid = document.getElementById('crosssell-grid');
    grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;font-weight:700;padding:1rem;opacity:0.5;">Loading...</p>';

    // Gather all products from the DOM cards
    var allCards = document.querySelectorAll('.view-details-btn');
    var related = [];
    allCards.forEach(function(btn) {
      var pid = btn.dataset.pid;
      if (pid == currentProductId) return; // skip self
    ...
    if (related.length === 0) {
      grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;font-weight:700;padding:1rem;opacity:0.5;">No similar products found.</p>';
      return;
    }
    ...
    related.forEach(function(p) {
      ...
      card.className = 'modal-crosssell-card';
      card.style.cursor = 'pointer';
      card.innerHTML =
        '<img src="' + p.img + '" alt="' + p.name + '" style="object-fit:cover;width:100%;aspect-ratio:1;border-radius:4px;"/>' +
        '<div class="modal-crosssell-card-name">' + p.name + '</div>' +
        '<div class="modal-crosssell-card-price">' + p.price + '</div>';

      // Click anywhere on card to open its modal
      card.addEventListener('click', function() {
        openModal(p.pid, p.name, p.price, p.img, p.desc, p.badge, p.sticker, p.cat, p.stock, p.rating, p.sku);
      });
